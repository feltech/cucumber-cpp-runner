...
// configuration step. It creates a namespace called `cucumber_cpp_runner`. You can modify
		...
		for (std::size_t cur_x = 0; cur_x < game_board.width; ++cur_x)
		{
			for (std::size_t cur_y = 0; cur_y < game_board.height; ++cur_y)
			{
				buttons.push_back(ftxui::Button(
					&game_board.get_string(cur_x, cur_y),
					[=, &game_board]
					{
...
struct Bitmap : ftxui::Node
{
	...
	void Render(ftxui::Screen & screen) override
	{
		for (std::size_t cur_x = 0; cur_x < width_; ++cur_x)
		{
			...
			{
				auto & pixel = screen.PixelAt(
					box_.x_min + static_cast<int>(cur_x), box_.y_min + static_cast<int>(cur_y));
				pixel.character = "▄";
				const auto & top_color = at(cur_x, cur_y * 2);
				const auto & bottom_color = at(cur_x, cur_y * 2 + 1);
				pixel.background_color =
					ftxui::Color{top_color.R.get(), top_color.G.get(), top_color.B.get()};
				pixel.foreground_color =
					ftxui::Color{bottom_color.R.get(), bottom_color.G.get(), bottom_color.B.get()};
			}
		}
	}
...
void game_iteration_canvas()
{
	...
	auto game_iteration = [&](const std::chrono::steady_clock::duration elapsed_time)
	{
		...
		auto & small_bm_pixel = small_bm->data().at(
			static_cast<std::size_t>(elapsed_time.count()) % small_bm->data().size());
	...
	auto make_layout = [&]
	{
		...
		return ftxui::hbox(
			{bm | ftxui::border,
			 ftxui::vbox(
				 {ftxui::text("Frame: " + std::to_string(counter)),
...
int main(int argc, const char ** argv)
{
	try
	{
		CLI::App app{fmt::format(
			"{} version {}",
			cucumber_cpp_runner::cmake::project_name,
			cucumber_cpp_runner::cmake::project_version)};
